...
public class ConvertRepositoryPropertyAspect {
	...
	public Object around(ProceedingJoinPoint proceedingJoinPoint, ConvertResponseProperty convertResponseProperty)
			throws Throwable {
		...
		if (convertResponseProperty.isArray()) {
			...
			for (int i = 0; i < jsonArray.size(); i++) {
				...
				if (jsonObject.containsKey(convertResponseProperty.fields())) {
					jsonObject.put(convertResponseProperty.fields(),
							String.valueOf(jsonObject.get(convertResponseProperty.fields())));
				}
