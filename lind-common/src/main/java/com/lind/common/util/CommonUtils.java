...
public class CommonUtils {
     ...
     * 平均分配.
     *
     * @param salesPerson  销售ID：剩余库容
     * @param customerObjs 分配对象列表
     */
    public static <T> Map<String, List<T>> allocateAvg(Map<String, Integer> salesPerson,
                                                       List<T> customerObjs) {
        int salesPersonNum = salesPerson.size();
        if (salesPersonNum == 0) {
        ...
        List<String> salesPersonIds =
                salesPerson.entrySet()
                        .stream()
                        .map(stringIntegerEntry -> stringIntegerEntry.getKey())
                        .collect(Collectors.toList());
        while (customerNum >= 1) {
            String salesId = salesPersonIds.get(--salesPersonNum);
            List<T> allocatedObjs = allocations.get(salesId);
            if (CollectionUtils.isEmpty(allocatedObjs)) {
                allocatedObjs = new ArrayList<>();
            }
            allocatedObjs.add(customerObjs.get(--customerNum));
            allocations.put(salesId, allocatedObjs);
            if (Objects.equals(allocatedObjs.size(), salesPerson.get(salesId))) {
                salesPersonIds.remove(salesId);
                salesPersonNum--;
            }
            salesPersonNum = salesPersonNum <= 0 ? salesPersonIds.size() : salesPersonNum;
        }
        return allocations;
    }
