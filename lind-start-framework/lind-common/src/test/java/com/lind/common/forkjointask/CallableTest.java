...
/**
 * 此示例程序利用一个执行器来计算多个长整型的和。内部 Sum 类实现了 Callable 接口，执行器将该接口用于结果计算，并发工作是在 call() 方法内部执行的。java.util.concurrent.Executors 类提供了一些实用方法，如提供预配置的执行器或将普通的旧 java.lang.Runnable 对象包装到 Callable 的实例中。与 Runnable 相比，使用 Callable 的优势是可以显式返回一个值。
 * 此示例使用了一个执行器向两个线程分派工作。ExecutorService.invokeAll() 方法接收 Callable 实例的集合，并在返回之前等待所有实例的完成。它返回一个 Future 对象的列表，所有这些对象都代表计算的“未来”结果。如果以异步方式工作，可以测试每个 Future 对象，检查其对应的 Callable 是否已经完成工作，并检查它是否抛出了一个异常，甚至可以将其取消。相比之下，使用普通传统线程时，必须通过一个共享的可变布尔值来编码取消逻辑，并通过定期检查这个布尔值来废除代码。由于 invokeAll() 导致阻塞，我们可以直接遍历 Future 实例并获取其计算和。
 * 还需注意的是，必须关闭执行器服务。如果未关闭，当主方法退出时，Java 虚拟机将不会退出，因为环境中仍有活跃的线程。
 */
