...
public class Java8StreamTest {
	...
	/**
	 * 1.将Person转换成PersonInfo类型的集合
	 * 2.过滤出薪水大于2000的人
	 */
	@Test
	public void mapReduce() {
		List<Integer> array = Arrays.asList(3, 4, 2, 1, 5);
		int result = array.parallelStream()
				.filter(o -> o % 2 == 0)
				.sorted()
				.map(x -> x * 2)
				.reduce(1, (e, c) -> e + c);
	...
	public void objectMap() {
		...
		List<PersonInfo> personInfoList = personInfos.stream()
				.map(PersonInfo.class::cast)
				.map(mapper -> Arrays.stream(mapper.getHobbies())
						.filter(type -> Objects.equals("足球", type))
