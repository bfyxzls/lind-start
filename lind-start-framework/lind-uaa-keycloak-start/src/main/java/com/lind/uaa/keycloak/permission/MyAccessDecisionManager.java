  ...
  public void decide(Authentication authentication, Object o, Collection<ConfigAttribute> configAttributes) throws AccessDeniedException, InsufficientAuthenticationException {
    ...
    while (iterator.hasNext()) {
      ...
      for (GrantedAuthority ga : authentication.getAuthorities()) {
        //获取当前用户token里的权限字段
        String userAuth = RoleUtil.removeRolePrefix(ga.getAuthority(), keycloakSpringBootProperties.getResource());
        if (needPerm.equals(userAuth)) {
          return;
        } else {
          //通过角色取它的权限列表
          List<? extends ResourcePermission> permissionList = permissionServiceManager.getPermissionByRoleFromCache(userAuth);
